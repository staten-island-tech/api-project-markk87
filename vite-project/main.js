...
gsap.from("#usdToCurrencyForm", {
  duration: 3,
  y: "-300%",
  ease: "expo",
  delay: 1.5,
});
gsap.from("#convertFactors", {
  duration: 1,
  x: "-500%",
  ease: "expo",
  delay: 2,
...
async function api() {
  ...
  try {
    ...
    if (data.result === "success") {
      ...
      const conversionFactorsContainer =
        document.querySelector("#convertFactors");
      Object.keys(data.rates).forEach((currency) => {
        conversionFactorsContainer.insertAdjacentHTML(
          "beforeend",
          `<li id="conversionFactors">${currency}: ${data.rates[currency]}</li>`
        );
      });
    ...
    if (error != 200 && error >= 400) {
      ...
      document.getElementById("result").textContent =
        "An error occurred. Please make sure you entered all the values.";
    }
  }
}
api();
...
async function convertUSD(data) {
  ...
  document.getElementById(
    "result"
  ).textContent = `${usdAmount} USD is approximately ${convertedAmount} ${currencyCode.value}`;
}
